...
void RAS_BucketManager::RenderSortedBuckets(RAS_Rasterizer *rasty, RAS_BucketManager::BucketType bucketType)
{
	...
	if (leafs.size() > 0) {
		...
		const mt::vec3 pnorm(m_nodeData.m_trans.getBasis()[2]);
...
void RAS_BucketManager::Renderbuckets(const mt::mat4x3& cameratrans, RAS_Rasterizer *rasty, RAS_OffScreen *offScreen)
{
