...
void RAS_MeshSlot::RunNode(const RAS_MeshSlotNodeTuple& tuple)
{
	...
	// TODO need matrix update bool cached in a node argument
	if (/*(!m_pDeformer || !m_pDeformer->SkipVertexTransform()) &&*/ !istext) { // TODO deformer can be moved in node argument from DAB.
		float mat[16];
